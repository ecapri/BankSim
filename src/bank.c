...
int bank(int atm_out_fd[], Command *cmd, int *atms_remaining) {
  ...
  // check that the ATM ID unpacked from the received command is a valid ATM
  if (check_valid_atm(i) != SUCCESS) {
    return ERR_UNKNOWN_ATM;
  }
  
  // CONNECT, EXIT, DEPOSIT, WITHDRAW, TRANSFER, and BALANCE
  if (c == CONNECT) {
      ...
      error_msg(ERR_UNKNOWN_CMD, "Unknown command, invalid message to bank.\n");
